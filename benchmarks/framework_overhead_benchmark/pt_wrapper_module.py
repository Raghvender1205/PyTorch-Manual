...
class WrapperModule(object):
    r"""
    Wraps the instance of wrapper_type.
    For graph_mode traces the instance of wrapped_type.
    Randomly initializes num_params tensors with single float format.

    Args:
        wrapped_type:
            - Object type to be wrapped.
                Expects the wrapper_type to:
                    1. be constructed with pt_fn specified in module_config.
                    2. provide forward method that takes module_config.num_params args.
        
        module_config: 
            - Specified pt_fn to construct wrapped_type with, whether graph_mode is enabled,
                and number of parameters wrapped_type's forward method takes
