...
def get_deepspeech(device: torch.device) -> GetterReturnType:
    ...
    window_size = 0.02
    window = 'hamming'
    audio_conf = dict(sample_rate=sample_rate, window_size=window_size,
                        window=window_size, noise_dirn=None)
    
    N = 10
    num_classes = 10
    spectrogram_size = 161
    seq_length = 500 # Original -> 1343
    target_length = 10 # Original -> 50
    ...
    def forward(*new_params: Tensor) -> Tensor:
        ...
        out = out.transpose(0, 1) # For CTCLoss
...
def get_multiheadattn(device: torch.device) -> GetterReturnType:
    ...
    in_proj = models.InProjContainer(nn.Linear(embed_dim, embed_dim, bias=False),
                                     nn.Linear(embed_dim, embed_dim, bias=False),
                                     nn.Linear(embed_dim, embed_dim, bias=False))

    model = models.MultiheadAttentionContainer(nhead, in_proj, models.ScaledDotProduct(), nn.Linear(embed_dim, embed_dim, bias=False))
