...
class Wav2Letter(nn.Module):
    r"""
        Wav2Letter Model architecture from `"Wav2Letter: an End-to-End ConvNet-based Speech Recognition System"
        ...
        Args: 
            ...
            - num_features (int, optional): Number of input features that the network will recieve (Default: ``1``).
    """
    def __init__(self, num_classes: int = 40, input_type: str = "waveform", num_features: int = 1) -> None:
        ...
        if input_type == 'waveform':
            waveform_model = nn.Sequential(
                nn.Conv1d(in_channels=num_features, out_channels=250, stride=160, padding=45),
                nn.ReLU(inplace=True)
            )
...
class MaskConv(nn.Module):
    def __init__(self, seq_module):
        r"""
        Adds padding to the output of the module based on the given lengths. This is to ensure that the results 
        of the model do not change when batch size change during inference.

        Input needs to be in the shape of (B x C x D x T)
...
class Lookahead(nn.Module):
    ...
    Args:
        input_shape: sequence, batch, feature -> T x N x H
    ...
    def __init__(self, n_features, context):
        ...
        self.conv = nn.Conv1d(self.n_features, self.num_features, kernel_size=self.context, stride=1,
...
class DeepSpeech(nn.Module):
    def __init__(self, rnn_type, labels, rnn_hidden_size, nb_layers, audio_conf, bidirectional, context=20):
        ...
        window_size = self.audio_conf['window_rate']
        ...
        rnn_input_size = int(math.floor(rnn_input_size + 2 * 10) / 2 + 1)
        rnn_input_size *= 32
        ...
        self.lookahead = nn.Sequential(
            # NOTE: Consider adding Batch Norm
    ...
    def get_seq_lens(self, input_length):
        """
        Given 1D Tensor or Variable containing integer sequence lengths, return a 1D Tensor or variable
...
class PositionalEncoding(nn.Module):
    r"""
        Inject some information about relative or absolute position of tokens 
        in the sequence. The positional encoding have the same dimension as the embeddings, 
        so they could be summed together. Here, we use sine and cosine functions of different frequencies.
...
class TransformerModel(nn.Module):
    """ Contains module with an encoder, a recurrent or transformer module, and a decoder. """
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, dropout=0.5):
        super(TransformerModel, self).__init__()
        try:
            from torch.nn import TransformerEncoder, TransformerEncoderLayer
        except:
            raise ImportError('TransformerEncoder module does not exist.')
...
class MultiheadAttentionContainer(nn.Module):
    def __init__(self, nhead, in_proj_container, attention_layer, out_proj):
        r"""
        A Multihead attention container
        Args:
            nhead: The number of heads in the multihead attention model
            in_proj_container: A container of multihead in-projection linear layers (NOTE: nn.Linear)
            attention_layer: Attention Layer
            out_proj: The multi-head out-projection layer (NOTE: nn.Linear)
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, attn_mask: Optional[torch.Tensor] = None,
                bias_k: Optional[torch.Tensor] = None, bias_v: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        r"""
        Args:
            query, key, value (Tensor): map a query and a set of key-value pairs to an output.
            attn_mask, bias_k, bias_v (Tensor, Optional): keyword args (**kwargs) passed to the attention layer.
        ...
        assert k.size(-1) % self.nhead == 0, "key's embed_dim must be divisble by the number of heads" 
