...
import elementwise    # noqa: F401
import matmul         # noqa: F401
# from . import normalization  # noqa: F401
# from . import pooling        # noqa: F401
import reduction      # noqa: F401
import softmax        # noqa: F401
import rnn_eltwise    # noqa: F401
import swish          # noqa: F401
        ...
        for mode, device, dtype, config in itertools.product(
            ...
            if not bench.is_supported():
                if allow_skip:
                    ...
                    raise ValueError(
                        "attempted to run an unsupported benchmark: %s" % (
                            bench.desc())
                    )
            bench.run(args)
    ...
    def run_with_input_iter(bench_cls, input_iter, allow_skip=True):
        ...
        for start, stop, inc in tensor_dim_specs:
            ...
            if inc == 'pow2':
                ...
                dim_list = list(
                    range(int(start), int(stop) + int(inc), int(inc)))
        ...
        for name in args.benchmark_names:
            ...
            for bench_cls in benchmark_classes:
                if name in bench_cls.module():
                    ...
                    if (args.input_iter is not None) and bench_cls.input_iterable():
                        run_with_input_iter(
                            bench_cls, args.input_iter, allow_skip=True)
                    else:
                        if args.input_iter is not None:
                            print(
                                "WARNING: Incompatible benchmark class called with input_iter arg: {}".format(name))
            ...
            for bench_cls in benchmark_classes:
                ...
                if name.startswith(cls_module):
                    ...
                    bench = bench_cls(
                        config[0], config[1], torch.float32, *config[2:])
                    bench.jit_mode = args.jit_mode
                    bench.output_type = args.output
                    bench.run(args)
