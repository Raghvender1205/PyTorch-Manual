...
std::ostream& operator<<(std::ostream&, const xs& x) {
    bool first = true;
...
struct EnableMklDnn {
    explicit EnableMklDnn(bool enable) : prev_(at::globalContext().userEnabledMkldnn()) {
        at::globalContext().setUserEnableMkldnn(enable);
    }
...
static void BM_conv2d_native(benchmark:State& state, const ConvParams& params) {
...
static void BM_conv2d_mkldnn(benchmark::State& state, const ConvParams& params) {
    auto input = at::randn(params.input);
    auto weight = at::randn(params.weight);
    auto bias = params.bias.size() > 0 ? at::randn(params.bias) : at::Tensor{};
    ...
    if (Reorder == WeightAndInput) {
        ...
        auto r = ideep::tensor(params.input, ideep::data_type::f32, ideep::format_tag::aBcd16b);

        it_input.reorder_to(r);
        input = at::native::new_with_itensor_mkldnn(std::move(r), at::kFloat, at::Device(at::kCPU));
    }
    ...
    auto output = at::mkldnn_convolution(input, weight, bias, params.padding, 
            params.stride, params.dilation, params.groups);
    
    for (auto _ : state) {
        output = at::mkldnn_convolution(input,
        weight,
        bias,
        params.padding,
        params.stride,
    ...
    state.counters["GFLOPS/s"] = benchmark::Counter(
        2.0f * output.numel() * weight.numel() / weight.size(0) * state.iterations(), benchmark::Counter::kIsRate);
    state.counters["GB/s"] = benchmark::Counter(
        // NOLINTNEXTLINE(cppcoreguidelines-narrowing-conversions,bugprone-narrowing-conversions)
        state.iterations() * (input.nbytes() + weight.nbytes() + output.nbytes()), benchmark::Counter::kIsRate);
}
...
void registerOne(const char* base, const ConvParams& params) {
    benchmark::RegisterBenchmark(name(base, "native", params).data(), BM_conv2d_native<true>, params);
