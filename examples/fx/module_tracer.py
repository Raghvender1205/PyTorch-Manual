"""
Recording Module Hiearchy with a Custom Tracer
...
from which that operation originated. The _qualified_name_ is the path to the Module
...
class ModulePathTracer(torch.fx.Tracer):
    """
    ModulePathTracer is an FX Tracer that for each operation also records the
    qualified name of the Module from which the operation originated
    """
    ...
    def create_proxy(self, kind: str, target: torch.fx.node.Target, args: Tuple[Any, ...], 
        ...
        """
        Override of `Tracer.create_proxy`. This override intercepts the recording of every operation
        and stores away the current traced module's qualified name in `node_to_originating_module`.
        """
...
# Testing: How it works on Torchvision ResNet18 Model
