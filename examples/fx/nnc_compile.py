...
The goal is to demonstrate an end-to-end example of using FX to lower a PyTorch model
to a backend codegen compiler. We will be using NNC (Neural Network Compiler)

To do this, this example has 2 FX transformations. 

-> The first one is decomposition pass that normalizes and decomposes PyTorch operations (like addmm).
   Using a pass like this allows us to reduce the number of lowerings we need to write. Instead of needing 
   to specifically write a lowering for addmm, we can decompose addmm and lower its consistuent operations.

-> The second one is the actual lowering pass. In this case, we will need to convert each PyTorch operation
   we encounter into corresponding NNC (`TensorExpr`). 


These 2 passes, `decompose` and `nnc_compile` are very similar.
In both cases, we reinterpret each operation in the FX graph to convert an entirely new representation.
In the decomposition pass, we either copy the operation as-in into a new Graph, or we use `Proxy` objects
to decompose the operation.

In the lowering pass, a similar principle applies. However, instead of using `Proxy` objects to rewrite our op
in other PyTorch ops, we do the translation ourselves. In addition, since this is not the source-to-source
transformation, we return a somewhat hacky function that passes in the module attributes to that NNC Callable.

RESULTS
Using NNC (compiles directly to LLVM), we can compile a fairly small PyTorch model and compare performance between
NNC, PyTorch Eager and Static Runtime.
...
def decompose(model: torch.nn.Module, example_inputs) -> torch.nn.Module:
    """
    decompose(model, example_inputs) takes in a model, decomposes any of the functions in `decomposition_rules` to its constituent operations
    and returns a `nn.Module` without any of the operations with decomposition rules.
    """
    # Run it multiple times so we converge to a fixed point.
    for _ in range(5):
        model = fx.symbolic_trace(model)
        ShapeProp(model).propogate(*example_inputs)
        new_graph = fx.Graph()
        env = {}
        for node in model.graph.nodes:
            if node.op == 'call_function' and node.target in decomposition_rules:
                # If current function is in `decomposition_rules`, we use
                # `Proxy` objects to decompose the operations using 
                # decomposition rule.
                proxy_args = map_arg(node.arg, lambda n: fx.Proxy(env[n.name]))
...
def get_unary_nnc(op):
    def get_op_nnc(inp_shapes, args):
        def f(*idxs):
...
for torch_op, nnc_fn in unary_lowerings:
    lowering_functions[torch_op] = wrap_compute(get_unary_nnc(nnc_fn))
...
def nnc_compile(model: torch.nn.Module, example_inputs) -> torch.nn.Module:
    ...
    as `model.forward`, with an extra arg corresponding to where the output is stored.
    This function takes the inputs (which must be PyTorch tensors with the same shapes as example_inputs).
    It passes them to an NNC Executor.
    """
    fx_model = fx.symbolic_trace(model)
    ShapeProp(fx_model).propogate(*example_inputs)

    # This env maps from nodes to `te.ExprHandle`, which represent the output of an NNC Computation
    env = {}
    ...
    for node in fx_model.graph.nodes:
        if node.op == 'placeholder':
            ...
            env[node.name] = te.Placeholder(
                node.name, get_te_type(node), shapes)
            ...
            result = lower_function(
                node, node.target, lookup_env(node.args), node.args)
    ...
    cg = te.construct_codegen('llvm', stmt, [te.BufferArg(x) for x in [
                              env[i.name] for i in module_attrs] + inputs + outs])
