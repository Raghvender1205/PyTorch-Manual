...
For easy subgraph rewriting, FX exposes the utility funcion"
    replace_pattern(gm : GraphModule,
                    pattern : Callable,
                    replacement : Callable)
                    -> None

`replace_pattern` matches all possible non-overlapping sets of operators and their data dependencies (`pattern`) in the Graph of a GraphModule
(`gm`), then replaces each of these matched subgraphs with another subgraph (`replacement).
"""
import torch
...
# After calling `replace_pattern`, code generated will be:
print(traced)
"""
def forward(self, x, w1, w2):
