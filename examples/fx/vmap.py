...
def move_bdim_to_front(x, result_ndim=None):
    """
    Returns a tensor with batch dimension at the front. If a batch
    dimension already exists the move it. Otherwise, create a new batch dimentsion 
    at the front. If `result_ndim` is not None, ensure that the resulting tensor has rank value equal to `result_ndim`.
    """
    x_dim = len(x.shape)
    x_bdim = x.bdim
    if x_bdim is None:
