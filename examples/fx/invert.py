...
# An Inverse mapping is one that takes a function f(x) and returns a funtion g
# such that f(g(x)) == x. Like, log(exp(x)) == x, exp and log are inverses.
...
# The general strategy is that we walk the graph backwards, transforming each node
# into its inverse. To do this, we swap the outputs and inputs of the functions, 
# and then we look up to its inverse in `invert_mapping`.

# NOTE: This Transform assumes that all operations take in only one input and return one output.
